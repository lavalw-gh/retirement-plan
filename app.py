...
other_income = st.sidebar.number_input("Other Income (£)", 0.0, 1_000_000.0, 0.0, step=1000.0)
other_income_years = st.sidebar.number_input("Years", 0, 100, 0)
st.sidebar.caption("Fixed nominal income (no inflation) starting from current age.")
...
st.dataframe(df.style.format({col: "{:,.2f}" for col in num_cols}))

num_cols = scenario_summary.select_dtypes(include="number").columns
st.dataframe(scenario_summary.style.format({col: "{:,.2f}" for col in num_cols}))
...
def show_help_dialog():
    ...
    * **Other Income:** A fixed annual income stream (e.g., rental income, part-time work) that reduces the amount you need to withdraw from your portfolio. It lasts for the specified number of years and does not grow with inflation.
