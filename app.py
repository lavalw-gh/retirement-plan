...
current_isa = st.sidebar.number_input(
    "Current ISA / accessible (£)", 0.0, 10_000_000.0, 0.0, step=1_000.0
)
current_pension = st.sidebar.number_input(
    "Current pension (£)", 0.0, 10_000_000.0, 0.0, step=1_000.0
)

desired_pension_at_end = st.sidebar.number_input(
    "Desired pension at end of life (£, today's money)",
    0.0,
    10_000_000.0,
    0.0,
    step=10_000.0,
...
st.sidebar.markdown(
    "_Set desired pension to £0 for minimum capital, or e.g. £100,000 for a longevity buffer._"
)
