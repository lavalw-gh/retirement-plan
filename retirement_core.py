...
def size_capital(inputs: Inputs) -> Tuple[Results, pd.DataFrame, pd.DataFrame]:
    ...
    years_access_to_end = max(0, inputs.life_expectancy - inputs.pension_access_age + 1)
    pv_end_at_access = (
    ...
    # - before access: grow both, then withdraw from ISA
    # - after access: withdraw from pension, then grow remaining pension
    ...
    for t in range(inputs.life_expectancy - inputs.current_age + 1):
        ...
        if age < inputs.pension_access_age:
            # Accumulate both, withdraw from ISA
            ...
            # ISA still grows (usually near zero), pension: withdraw THEN grow
            bal_isa *= (1 + inputs.nominal_return)
...
def plot_single_projection(
    ...
    # Balances
    ax = axes[0]
    ax.plot(df["Age"], df["Total_Balance"], lw=2.6, label="Total", color="#2E86AB")
    ax.plot(df["Age"], df["ISA_Balance"], lw=1.8, label="ISA", color="#A23B72")
    ax.plot(df["Age"], df["Pension_Balance"], lw=1.8, label="Pension", color="#F18F01")
...
def plot_scenario_set(base: Inputs, out_png: str) -> pd.DataFrame:
    ...
    scenarios.append(
        (
            "Conservative (SWR 3.5%)",
            Inputs(**{**asdict(base), "swr": 0.035, "sizing_method": "SWR"}),
        )
    )
    scenarios.append(
    ...
    scenarios.append(
        (
            "Higher inflation",
            Inputs(**{**asdict(base), "inflation_rate": base.inflation_rate + 0.005}),
        )
    )
    scenarios.append(
        (
            ...
            Inputs(
                **{
                    **asdict(base),
                    "nominal_return": max(-0.5, base.nominal_return - 0.01),
                }
            ),
        )
    )
...
def create_pension_report(
    ...
    cells = [
        ("ISA / Accessible capital needed (today)", f"£{results.isa_needed_today:,.0f}"),
        ("Pension pot needed (today)", f"£{results.pension_needed_today:,.0f}"),
        ("Total capital needed (today)", f"£{results.total_needed_today:,.0f}"),
        (
    ...
    if inputs.desired_pension_at_end > 0:
        approx_years = int(
            inputs.desired_pension_at_end / max(inputs.annual_spending_today, 1)
        )
        commentary_paras.append(
    ...
    assumptions_data = [
        ...
        ("Annual spending target (today)", f"£{inputs.annual_spending_today:,.0f}"),
        ("Inflation rate (constant)", f"{inputs.inflation_rate:.2%} per year"),
        ("Nominal investment return (constant)", f"{inputs.nominal_return:.2%} per year"),
        ("Real return (inflation-adjusted)", f"{results.real_return:.2%} per year"),
    ...
    if scenario_png and os.path.exists(scenario_png):
        ...
        doc.add_paragraph(
            ...
            "higher inflation, lower returns). This illustrates the range of capital requirements under different conditions."
        )
